...
export const bannerVariants = tv({
  ...
  variants: {
    ...
    variant: {
      default: { root: "bg-muted text-foreground [--banner-tone:var(--foreground)] [--surface:var(--muted)]", icon: "text-muted-foreground", action: "text-foreground" },
      brand: { root: "bg-brand/10 text-foreground [--banner-tone:var(--brand)] [--surface:color-mix(in_oklab,var(--brand)_10%,var(--background))]", icon: "text-brand", action: "text-brand" },
      purple: { root: "bg-purple/10 text-foreground [--banner-tone:var(--purple)] [--surface:color-mix(in_oklab,var(--purple)_10%,var(--background))]", icon: "text-purple", action: "text-purple" },
      pink: { root: "bg-pink/10 text-foreground [--banner-tone:var(--pink)] [--surface:color-mix(in_oklab,var(--pink)_10%,var(--background))]", icon: "text-pink", action: "text-pink" },
      teal: { root: "bg-teal/10 text-foreground [--banner-tone:var(--teal)] [--surface:color-mix(in_oklab,var(--teal)_10%,var(--background))]", icon: "text-teal", action: "text-teal" },
      orange: { root: "bg-orange/10 text-foreground [--banner-tone:var(--orange)] [--surface:color-mix(in_oklab,var(--orange)_10%,var(--background))]", icon: "text-orange", action: "text-orange" },
      info: { root: "bg-info/10 text-foreground [--banner-tone:var(--info)] [--surface:color-mix(in_oklab,var(--info)_10%,var(--background))]", icon: "text-info", action: "text-info" },
      success: { root: "bg-success/10 text-foreground [--banner-tone:var(--success)] [--surface:color-mix(in_oklab,var(--success)_10%,var(--background))]", icon: "text-success", action: "text-success" },
      warning: { root: "bg-warning/10 text-foreground [--banner-tone:var(--warning)] [--surface:color-mix(in_oklab,var(--warning)_10%,var(--background))]", icon: "text-warning", action: "text-warning" },
      destructive: { root: "bg-destructive/10 text-foreground [--banner-tone:var(--destructive)] [--surface:color-mix(in_oklab,var(--destructive)_10%,var(--background))]", icon: "text-destructive", action: "text-destructive" },
    },
    ...
    // The bar's footprint. `bar` (default) runs edge to edge, the classic strip atop a page.
    // `inset` is the same bar drawn as a rounded card that sits INSIDE the page gutter, above the
    // navbar: place it in a `SectionContainer` (or any gutter) and it takes the gutter's width. Its
    // inner row drops the page-width cap and gutter (the card is already inside one) for a card
    // interior, and gains a step of height so the rounded shape has room to read as a card.
    shape: {
      bar: {},
      inset: { root: "rounded-md", inner: "max-w-none px-4 py-3" },
    },
  },
  compoundVariants: [
...
export function Banner({
  ...
  const slots = bannerVariants({ variant, appearance, align, dismissible, shape })
  return (
