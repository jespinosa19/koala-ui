...
export const treeVariants = tv({
  slots: {
    ...
    // Caret flips 90° on open; `caret-turn` is the content-reveal beat, so glyph + height move as
    // one gesture. Reads the row's data-expanded, not Collapsible's data-state.
    caretIcon:
      "size-3.5 caret-turn group-data-[expanded=true]/row:rotate-90",
    ...
    // tabular-nums so bare counts line up down the tree.
    actions: "ms-auto flex shrink-0 items-center gap-1 ps-2 tabular-nums text-muted-foreground",
