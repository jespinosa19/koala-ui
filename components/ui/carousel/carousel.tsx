...
export const carouselVariants = tv({
  slots: {
    ...
    indicator: [
      ...
      "relative before:absolute before:-inset-x-[3px] before:top-1/2 before:h-10 before:-translate-y-1/2 before:content-[''] pointer-coarse:before:h-11",
    ],
  ...
  variants: {
    ...
    // Thumbnail tile size (variant="thumbnails" only). `sm` is the 56x40 landscape strip for photo
    // slides; `lg` is a 64px square for a product gallery, where every slide is a square packshot
    // and a landscape crop would cut the object. Tile and gliding ring resize together, and the ring
    // still travels in its own widths (translateX(100% + gap)), so any size lines up.
    thumbSize: {
      sm: {},
      lg: { thumb: "h-16 w-16 rounded-lg", thumbRing: "h-16 w-16 rounded-lg" },
    },
    side: {
      left: { arrow: "left-2" },
      right: { arrow: "right-2" },
...
export interface CarouselIndicatorsProps extends React.ComponentProps<"div"> {
  ...
  /**
   * Thumbnail tile size, for `variant="thumbnails"`: `sm` (56x40, for landscape photos) or `lg`
   * (a 64px square, for product packshots). @default "sm"
   */
  thumbnailSize?: "sm" | "lg"
  /**
  ...
  return (
    ...
    >
      {variant === "thumbnails" && (
        <span
          aria-hidden
          className={slots.thumbRing({ thumbSize: thumbnailSize })}
        ...
        if (variant === "thumbnails") {
          return (
            <button
              ...
              aria-current={active || undefined}
              onClick={() => setIndex(i)}
              className={slots.thumb({ variant, active, thumbSize: thumbnailSize })}
            >
              {thumbnails?.[i]}
            </button>
          )
