...
export const pricingVariants = tv({
  slots: {
    ...
    description: "text-pretty text-sm text-body",
    ...
    // The price is display type, not UI text: it takes the heading face (DM Sans) at bold, like the
    // section title above it. It is a `span`, so it would otherwise fall to Inter, and a 48px Inter
    // figure under a DM Sans headline read as a different voice. `tabular-nums` still holds.
    amount: "font-heading text-4xl font-bold tracking-tight tabular-nums text-foreground",
    // tabular-nums: a period like "billed $288 yearly" swaps with the amount on the billing toggle.
    period: "text-sm font-medium tabular-nums text-muted-foreground",
  ...
  variants: {
    ...
    tone: {
      ...
      // card on a dashed contour, so it reads as open-ended next to the fixed-price tiers.
      dashed: { tier: "border-dashed" },
    },
    ...
    // The type scale of a tier. `md` is the compact card for a docs page, a dialog or a 3-up row.
    // `lg` is the landing-page scale: a tier read on its own, with the inventory set at body size
    // (16px rows, a 48px price, a readable group label) so a long categorised list does not turn
    // into fine print. One step, not a className per part, because the name, price, rows and group
    // labels have to move together or the card loses its hierarchy.
    size: {
      md: {},
      lg: {
        name: "text-xl",
        ...
        amount: "text-5xl",
        features: "text-base",
        featureGroupList: "gap-3 text-base",
        featureGroupLabel: "text-sm [&>svg]:size-5",
      },
    },
  },
  defaultVariants: {
...
export function PricingTier({
  ...
  const slots = pricingVariants({ featured, tone, actionPosition, size })
...
export interface PricingFeatureGroupProps extends Omit<React.ComponentProps<"div">, "children"> {
  /**
   * The group's centred header label, e.g. "What's inside" or "Some gifts included". Optional:
   * omit it and the group renders as a bare list, for a tier whose inventory is short enough that
   * a header would be noise.
   */
  ...
  return (
    <div data-slot="pricing-feature-group" className={slots.featureGroup({ className })} {...props}>
      {/* The header is a labelled Divider. `static` keeps it painted: it lands first in the group,
          where the smart :first-child rule would otherwise auto-collapse it. An unlabelled group
          drops the rule entirely, and with it the list's top margin, which only earns its keep
          separating the rows from a header. */}
      {label != null && (
        <Divider static>
      ...
      <ul className={slots.featureGroupList({ className: label == null ? "mt-0" : undefined })}>
        {children}
      </ul>
    </div>
  )
...
export interface PricingTierActionProps extends React.ComponentProps<"div"> {
  /**
   * Control height for the CTA. Pricing has no density axis of its own, so without this a plan's
   * button fell back to the ambient density and every pricing CTA rendered at the small tier. The
   * band imposes the house CTA (36px, see lib/density.tsx), the same button the hero, the navbar
   * and every section CTA use, so a plan's button is not a size of its own halfway down the page.
   * It reads big regardless: a tier CTA is full-width. The Button's own `size` still wins.
   * @default "md"
   */
  size?: ButtonSize
}

export function PricingTierAction({
  className,
  size = "md",
  children,
  ...props
