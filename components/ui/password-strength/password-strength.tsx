...
/**
 * The roll shared by both glyphs of a requirement's indicator: the dot lifts up and out, the check
 * rises from below into place. Three deliberate choices, all of them about making the travel
 * something you can actually *see* on a 16px glyph:
 *
 * 1. **No `scale`.** The icon-swap pop (scale 0.25) shrinks the glyph to a 4px dot for most of its
 *    journey, and a 4px dot has no legible direction. Here the movement is the message, so the
 *    glyph stays full size and only fades.
 * 2. **A different curve for the travel.** `--ease-out` is extremely front-loaded (~87% of the
 *    distance inside the first third), so the glyph lands before it is opaque enough to be seen
 *    moving — the rise reads as a blink. The travel runs on `--ease-drawer` instead, the DS's
 *    smooth-decelerate curve, which spreads the distance across the window it's visible in. Fade
 *    and blur keep `--ease-out`.
 * 3. **`translate` named explicitly**, because Tailwind v4 compiles `translate-*` to the
 *    standalone `translate` property, not to `transform` — under `transition-transform` it snaps.
 *
 * One `transition` shorthand rather than `transition-[…] duration-base` plus overrides, so the
 * per-property easing can't lose a cascade race. Tokens only: every duration and easing is a var.
 */
const ICON_ROLL =
  "absolute [transition:opacity_var(--duration-base)_var(--ease-out),translate_var(--duration-base)_var(--ease-drawer),filter_var(--duration-base)_var(--ease-out)]"
      ...
      {strength.rules.map((rule) => (
        ...
        >
          <span className={slots.itemIcon()} aria-hidden="true">
            {/* Dot → check on the same roll the verdict label uses, so the whole component moves in
                one direction: the dot lifts up and out while the check rises from below into place.
                Both stay mounted and absolutely stacked, so this is a transition rather than a
                keyframe: a rule that flips back mid-roll retargets instead of restarting, which
                matters when the list is reacting to every keystroke. */}
            <Circle
              weight="bold"
              className={`${ICON_ROLL} size-3 text-muted-foreground ${
                rule.met ? "-translate-y-2 opacity-0 blur-[4px]" : "translate-y-0 opacity-100 blur-[0px]"
              }`}
            />
            <Check
              weight="bold"
              className={`${ICON_ROLL} size-4 text-success ${
                rule.met ? "translate-y-0 opacity-100 blur-[0px]" : "translate-y-2 opacity-0 blur-[4px]"
              }`}
            />
          </span>
          {rule.label}
