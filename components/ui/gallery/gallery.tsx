...
export const galleryVariants = tv({
  slots: {
    ...
    header:
      "mx-auto flex w-full max-w-2xl flex-col items-center gap-5 px-6 pt-16 text-center sm:px-8 sm:pt-24",
    ...
    // Sides use the canonical section gutter (px-6 sm:px-8); the wide max-w is the intentional bleed.
    masonry:
      "mx-auto mt-12 w-full max-w-[110rem] columns-2 gap-4 px-6 pb-16 sm:columns-3 sm:px-8 sm:pb-24 lg:columns-4",
    // One framed tile. Concentric radius + hairline frame + a faint inset ring (the image-outline
    // polish principle). The frame is an `--edge` ring plus the xs lift, not a border ("Shadows
    // over borders"), so the photo fills to the tile's true radius. Hover deepens the shadow only,
    // never a position change.
    item: [
      "group relative mb-4 block break-inside-avoid overflow-hidden rounded-2xl bg-card shadow-xs ring-1 ring-edge",
      "transition-[box-shadow] duration-base ease-out hover:shadow-md",
    ...
    // The preview image: fills its tile. No hover transform, so the tile never shifts. No default
    // ratio on purpose: a masonry wall wants a *mix* of shapes, so the consumer passes one per tile
    // (`className="aspect-[4/5]"`, or real `width`/`height` attributes). Without either, the box
    // has no height until the photo decodes and the whole wall reflows as images land.
