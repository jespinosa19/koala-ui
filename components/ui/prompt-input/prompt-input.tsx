...
const promptInputVariants = tv({
  slots: {
    ...
    // Top-aligned, like Alert and Toast: on a narrow viewport the notice wraps to two lines and
    // a centered glyph would float at the block's midpoint instead of sitting on line one.
    // `bannerClose` opts back out with its own `self-center`.
    bannerRow: "flex items-start gap-2.5 px-4 py-3 text-sm",
    bannerIcon: "mt-px shrink-0 [&>svg]:size-5",
    ...
    // A single row that SCROLLS rather than wraps. Prompt starters are questions, not labels, so
    // on a phone even two of them are wider than the screen: wrapped, the second one still
    // overflowed its row (a chip is `shrink-0`, so it cannot give the width back), and the reader
    // met a sentence running off the edge. Scrolling is also how every chat client presents them.
    // `scroll-fade-x` is the canonical hint at what is still off-screen (globals.css) and is inert
    // when the chips already fit; `py-1` keeps the focus ring and the chips' shadow off the clip.
    suggestions: "flex gap-2 overflow-x-auto py-1 scroll-fade-x",
    suggestion: [
  ...
  variants: {
    ...
    size: {
      // `pointer-coarse:text-base` stops iOS zooming into the focused field (it zooms under 16px).
      sm: {
        textarea: "px-3 pt-2.5 pb-1 text-sm pointer-coarse:text-base min-h-10",
        toolbar: "px-2 pb-2 pt-0.5",
      },
      md: {
        textarea: "px-4 pt-3.5 pb-1.5 text-sm pointer-coarse:text-base min-h-12",
