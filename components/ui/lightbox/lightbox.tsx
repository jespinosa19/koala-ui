...
export const lightboxVariants = tv({
  slots: {
    ...
    // light photo doesn't bleed into the black. A real `outline` pulled 1px inside, not a ring: an
    // <img> paints its pixels over its own inset box-shadow, so `ring-inset` never showed (memory
    // `inset-ring-under-children`). Soft zoom-in on enter / per-image crossfade. The
    ...
    image: [
      "h-auto w-auto max-w-[90vw] min-h-0 rounded-lg object-contain shadow-2xl outline-1 -outline-offset-1 outline-white/10",
    ...
    //
    // `p-1`: setting `overflow-x` makes the y axis clip too (a non-`visible` axis forces the other to
    // `auto`), and the clip lands on the PADDING box. The selection ring and the focus ring are drawn
    // OUTSIDE the tile, so with zero padding their outer 2px is shaved off top and bottom, and at
    // both scroll ends. A little room all round leaves the ring whole; `scroll-p-1` keeps the
    // centering scroll from tucking the edge tiles' ring back under that padding.
    thumbsViewport: [
      "w-full max-w-[90vw] shrink-0 overflow-x-auto overscroll-x-contain p-1 scroll-p-1",
