...
import { useControlSize, useDensity, type ControlSize } from "@/lib/density"
...
// = (0,3,0) specificity, which beats the child's own rounded-pill at (0,1,0).
// The rounded-r-none / rounded-l-none longhand props override the rounded-pill
// shorthand on the child element through the normal cascade, so an attached group reads as one
// pill cut into segments: round outer ends, square seams.
  ...
  variants: {
    attached: {
      true: [
        ...
        // Share one border between neighbours by dropping the inner left edge. A -1px overlap
        // would stack two translucent strokes into a darker seam (FOUNDATIONS.md, "Alpha strokes").
        "[&>[data-slot=button-group-item]:not(:first-child)]:border-l-0",
        // Lift the focused item so its ring paints over the neighbour that follows it.
...
// The control sizes only: a group is a toolbar or a segmented control, never a marketing CTA, so
// the button-only `xl` tier is not offered here.
type GroupSize = ControlSize
