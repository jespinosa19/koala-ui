...
export const datePickerTriggerVariants = tv({
  base: [
    // tabular-nums: the picked date is the label, so digits hold one width as it changes.
    "group inline-flex items-center gap-2 rounded-md border border-input text-left tabular-nums text-foreground",
  ...
  variants: {
    size: {
      ...
      sm: "h-8 px-2.5 text-sm pointer-coarse:text-base [&_svg]:size-4",
      md: "h-9 px-3 text-sm pointer-coarse:text-base [&_svg]:size-4",
      lg: "h-10 px-3.5 text-sm pointer-coarse:text-base [&_svg]:size-[18px]",
    },
    ...
    return (
      <div className={slots.root({ className })} data-slot="calendar">
        <div className={slots.header()}>
          ...
          >
            {/* One control whose label toggles in place between a year and a decade span
                ("2025" ⇄ "2020 – 2031"), so it rolls and the box eases between the two widths
                instead of the header jumping as the view changes. */}
            <AnimatedLabel swapKey={captionLabel}>{captionLabel}</AnimatedLabel>
          </button>
          <button
            type="button"
            aria-label="Next"
