...
const chatVariants = tv({
  slots: {
    ...
    reasoningSource: [
      "group/source inline-flex max-w-full cursor-pointer items-center gap-1.5 rounded-full border border-border bg-[var(--surface,var(--background))] px-2 py-1 text-xs text-foreground no-underline",
...
/**
 * How close to the bottom (px) still counts as "following the thread": roughly one bubble of
 * slack, so a reader who has scrolled up even slightly keeps their place.
 */
...
export interface ConversationProps extends React.ComponentProps<"div"> {
  ...
  variant?: ChatVariant
  /**
   * Follow new turns: keep the viewport pinned to the newest message as the thread grows, the way
   * every real chat client does. It only follows while the reader is *already* at the bottom, so
   * scrolling up to reread history is never yanked back down. Turn it off for a static transcript
   * that should open at the top. @default true
   */
...
export function Conversation({
  ...
  // `following`: is the reader parked at the bottom. `pinned`: has the first pass already run.
  // Refs, not state: both are read from a layout effect and a scroll handler, and neither should
  // re-render the thread on every wheel tick.
  const following = React.useRef(true)
  const pinned = React.useRef(false)

  // Compose the caller's ref in the commit phase rather than mutating a value passed into a hook
  // (the repo's react-hooks lint forbids the latter). Same shape as `lib/stagger.tsx`.
  ...
  // No dependency array on purpose: "a turn arrived" has nothing to name (children are opaque
  // nodes), so this reconciles the scroll offset after every render and bails in one comparison
  // when there is nothing to do. It scrolls the *viewport* (`scrollTo`, never `scrollIntoView`,
  // which would also drag the page the thread is embedded in). The first pass lands instantly, so
  // a thread that starts taller than its box simply opens *at* the newest turn; every later one
  // glides, and reduced motion drops back to the jump.
  React.useLayoutEffect(() => {
    const el = viewport.current
    const first = !pinned.current
    pinned.current = true
    if (!el || !stickToBottom || !following.current) return
    const top = el.scrollHeight - el.clientHeight
    if (top <= 0 || Math.abs(el.scrollTop - top) < 1) return
    el.scrollTo({ top, behavior: first || prefersReducedMotion() ? "auto" : "smooth" })
  })
...
export function MessageReasoningTrigger({
  ...
  return (
    ...
    >
      ...
      />
      {/* The default label flips in place on every stream start/stop, so it rolls. Only the
          WORDS are wrapped: the Brain and the caret flank them, and boxing the whole trigger
          would drag both into the swap and dissolve them instead of leaving one pulsing and the
          other turning. A caller passing their own children owns whether it rolls. */}
      <span>
        {children ?? (
          <AnimatedLabel swapKey={streaming ? "thinking" : "idle"}>
