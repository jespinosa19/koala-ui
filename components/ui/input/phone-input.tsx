...
// country-flag-icons exports one component per ISO2 code; index them by code. It spreads extra
// props onto the <svg> but types them as HTMLAttributes, so name `preserveAspectRatio` here.
const FLAGS = Flags as unknown as Record<
  string,
  React.ComponentType<{ className?: string; title?: string; preserveAspectRatio?: string }>
>
...
const phoneInputVariants = tv({
  slots: {
    ...
    caret: "size-3 shrink-0 caret-turn-fast group-data-[state=open]:rotate-180",
  ...
  variants: {
    size: {
      // The dial code sits in the typed field's line, so it follows the field to 16px on touch.
      sm: { trigger: "-ml-2.5 pl-2.5 pr-2.5 text-xs", dial: "text-sm pointer-coarse:text-base" },
      md: { trigger: "-ml-3 pl-3 pr-3 text-sm", dial: "text-sm pointer-coarse:text-base" },
      lg: { trigger: "-ml-3.5 pl-3.5 pr-3.5 text-sm", dial: "text-sm pointer-coarse:text-base" },
    },
  },
 ...
 * matching the icon rhythm used across Selects. The SVG fills the box and
 * `preserveAspectRatio="xMidYMid slice"` cover-crops it (SVG's `object-cover`), so the flag
 * always reaches the edge. The edge is an `after:` ring painted OVER the flag, black/white at
 * 10%, keeping mostly-white flags (Japan, Finland) legible. An inset ring on the wrapper paints
 * under the SVG and only showed where the flag fell short, as a gray gap.
 */
function CountryFlag({ iso2, className }: { iso2: string; className?: string }) {
  ...
  return (
    <span
      ...
      aria-hidden="true"
    >
      {Flag ? <Flag className="size-full" preserveAspectRatio="xMidYMid slice" /> : null}
    </span>
  )
}
...
export function PhoneInput({
  ...
  onValueChange,
  onChange,
  // No default: resolved below the shared way (own prop > container > ambient density). Pinning
  // "md" here made a phone field read a step short of the Inputs beside it in a `Form`. The
  // popover search box keeps its own "sm".
  size,
  hasError,
  disabled,
  id,
  ...
  // Resolve once and feed BOTH the root and the country trigger: the trigger's `-ml-*` has to
  // cancel exactly the root's `px-*`. Reading the raw prop left the trigger on the md default
  // inside an lg Form, so its hover fill stopped 2px short of the left border.
  const resolvedSize = useControlSize(size)
  const slots = phoneInputVariants({ size: resolvedSize })
