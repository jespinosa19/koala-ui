...
const countrySelectVariants = tv({
  ...
  variants: {
    ...
    size: {
      // 16px on touch, in step with the text fields beside it (see Select).
      sm: { trigger: "h-8 px-2.5 text-sm pointer-coarse:text-base" },
      md: { trigger: "h-9 px-3 text-sm pointer-coarse:text-base" },
 ...
 * rhythm used across Selects. `preserveAspectRatio="xMidYMid slice"` cover-crops the SVG so the
 * flag always reaches the edge; the edge itself is an `after:` ring painted OVER the flag
 * (black/white at 10%), keeping mostly-white flags (Japan, Finland) legible. Same as PhoneInput.
 */
...
export interface CountrySelectProps extends VariantProps<typeof countrySelectVariants> {
  /**
   * Spacing tier. Like every other control, it only picks the DEFAULT `size` (compact → "sm",
   * comfortable → "md") when none is given; an explicit `size` always wins.
   */
  density?: Density
...
export function CountrySelect({
  ...
  // The trigger is a form control like any other, so its height resolves the shared way (see
  // lib/density.tsx): inside a `Form` it stands level with the Inputs instead of pinning itself
  // to `md` and reading a step short of the fields stacked above it. The in-list search box below
  // keeps its explicit `size="sm"`: it belongs to the popover, not to the trigger's tier.
  const resolvedSize = useControlSize(size, density)
  const slots = countrySelectVariants({ size: resolvedSize })
