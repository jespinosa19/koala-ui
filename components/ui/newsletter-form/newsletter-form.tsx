 ...
 *
 * Neither layout paints a fill (fill is elevation, docs/FOUNDATIONS.md), so the nested Input
 * takes whatever ground the form sits on: the page, a muted band, a filled panel around it.
 */
export const newsletterFormVariants = tv({
  slots: {
    root: "w-full",
    // Card surface: concentric radius (rounded-2xl outer, controls stay rounded-md inside). The
    // edge is an `--edge` ring plus the xs lift, not a border (docs/FOUNDATIONS.md, "Shadows over borders").
    card: "flex flex-col rounded-2xl text-card-foreground shadow-xs ring-1 ring-edge",
  ...
  variants: {
    ...
    variant: {
      ...
      inline: { card: "gap-2 p-0 shadow-none ring-0" },
    },
  },
...
export function NewsletterForm({
  ...
  return (
    <div data-slot="newsletter-form" className={slots.root({ className })} {...props}>
