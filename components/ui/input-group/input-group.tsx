...
import { useControlSize, ControlSizeProvider, type Density } from "@/lib/density"
...
export const inputGroupVariants = tv({
  ...
  variants: {
    size: {
      ...
      lg: {
        root: "h-10 text-sm pointer-coarse:text-base",
...
export interface InputGroupProps extends React.ComponentProps<"div"> {
  /**
   * Shell height + affix sizing. The group IMPOSES it on the controls inside (see below), so a
   * segment only needs its own `size` to deliberately differ. @default "md"
   */
  size?: Size
  /**
  ...
  return (
    ...
    >
      <div
        ...
        {...props}
      >
        {/* The group imposes its resolved size on every segment (see lib/density.tsx), the way
            Form does for its fields: one `size` on the shell instead of repeating it on the field,
            the select and the button. The shell already equalizes HEIGHT (`items-stretch` + the
            `h-auto` strips above); what the segments were getting wrong on their own is the rest
            of the size — padding, text size, the suffix button. A segment's own `size` still wins.
            The provider renders no element, so the seam selectors above keep matching the real
            segments; it sits INSIDE the root so the group's own size still resolves from the
            container around it (a Form, an OrderSummaryPromo). */}
        <ControlSizeProvider size={resolvedSize}>{children}</ControlSizeProvider>
      </div>
    </InputGroupProvider>
  )
}
