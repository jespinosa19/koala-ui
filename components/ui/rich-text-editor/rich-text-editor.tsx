...
// The 12px corners (rounded-md) aren't a taste call, they're the concentric step both of this
// component's surfaces derive: the 20px frame pads its toolbar by 8px (20 = 12 + 8) and the 16px
// bubble pads by 4px (16 = 12 + 4). The standalone Toolbar lands on 8px corners for the same
// reason (its 12px band pads by 4px), which is why the two differ. Glyphs are 16px in both button
// slots; the slots then differ only in their box dimensions.
const controlBase = [...toolbarControlBase, "rounded-md", "[&_svg]:size-4"]

export const richTextEditorVariants = tv({
  slots: {
    // The field surface. Mirrors Input's focus treatment (brand hairline + soft halo) so the
    // editor reads as one field, and clips the rounded corners over toolbar + content.
    root: [
      "flex flex-col overflow-hidden rounded-xl bg-background text-foreground",
      // The hairline is an INSET RING, not a `border`. A border sits in the box model and pushes
      // the padding box in by 1px, so the toolbar's 8px padding would resolve to 7px and the 12px
      // chips would no longer nest concentrically inside the 20px frame (20 ≠ 12 + 7). A ring is a
      // box-shadow: it paints the same 1px edge without stealing layout, so 20 = 12 + 8 holds
      // exactly. Same reasoning (and same fix) as the standalone Toolbar's band.
      "inset-ring-1 inset-ring-input",
      "transition-[box-shadow] duration-fast ease-out",
      // Input's focus look, rebuilt from ring slots: the `brand-ring` utility sets `box-shadow`
      // outright, which would wipe the inset hairline out. Tailwind keeps the inset ring and the
      // outer ring in separate box-shadow slots, so the two compose; the halo reads the very same
      // --ring-brand token that `brand-ring` paints.
      "focus-within:inset-ring-brand focus-within:ring-3 focus-within:ring-[color:var(--ring-brand)]",
    ],
    // A wrapping toolbar that scrolls into a second row on narrow widths rather than clipping.
    // The 8px padding is what sets the chips' corner (20 − 8 = 12), so it stays fixed across
    // densities; it isn't spacing to taste. It carries no background of its own: a full-bleed
    // child painting one would cover the root's inset ring along the top edge.
    toolbar: "flex flex-wrap items-center gap-0.5 border-b border-border p-2",
    ...
    // bar of 28px controls. Concentric: the 16px surface pads by 4px, so the controls inside land
    // on 12px (16 = 12 + 4), which also nests the inline link editor's Input and Buttons, both
    // rounded-md, for free. Its hairline is an inset ring for the same reason the root's is: a
    // border would eat 1px of that padding. Enters with a fade+zoom from its lower edge (it sits
    // above the selection); exit is handled by the engine hiding it (subtly, #6).
    bubble: [
      "flex items-center gap-0.5 rounded-lg bg-popover p-1 inset-ring-1 inset-ring-border-soft",
    ...
    content: [
      ...
      // 16px on touch: iOS zooms into a focused editable region under 16px (see Input).
      "text-sm pointer-coarse:text-base text-muted-foreground",
      ...
      // H4 shares the body size and is set apart by weight + the foreground color alone, so
      // `font-sans` holds it in the body face while h1–h3 take the DM Sans heading face from
      // the base layer. Letting it swap too would separate it from the copy it sizes with.
      "[&_.tiptap_h4]:font-sans [&_.tiptap_h4]:text-sm [&_.tiptap_h4]:font-semibold [&_.tiptap_h4]:text-foreground",
  ...
  variants: {
    // Density tunes the WRITING AREA only (never radius, color, or the toolbar). Note that an
    // editor with no `density` prop and no provider resolves to `compact`, not to the
    // `defaultVariants` below: `useDensity` falls back to the DensityContext default, which is
    // `compact` (see lib/density.tsx). The defaultVariants only apply when this recipe is called
    // directly.
    //
    // The toolbar deliberately sits outside this axis, for two reasons. Its 8px padding is what
    // sets the chips' corner (20 − 8 = 12), so moving it with density would break the concentric
    // nesting in one of them. And a format bar that shrinks with FORM density becomes hard to hit,
    // which is why the standalone Toolbar keeps its own size scale rather than reading density
    // either. The controls hold 32px in both.
    density: {
      comfortable: { editable: "px-4 py-3" },
      compact: { editable: "px-3 py-2.5" },
    },
  },
  defaultVariants: {
    density: "comfortable",
