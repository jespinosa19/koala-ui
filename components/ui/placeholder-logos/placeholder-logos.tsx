...
/**
 * The stride a rotating wall walks the set in: slot `i` shows {@link PLACEHOLDER_BRANDS_VARIED}`[i +
 * swap * LOGO_ROTATE_SLOTS]`, so as long as a wall (or row) renders at most this many slots, the set
 * on screen and the set rolling out of it never share a brand. Keeps the social-proof grids a clean
 * 2x4.
 */
...
/**
 * The brisker hold, for a wall that is a FOOTNOTE rather than the section's subject. A mono strip
 * closing a hero band is small, low-contrast and easy to read as a flat image, so it earns a shorter
 * hold than a full-color social-proof wall the eye is already resting on. Pass it to
 * {@link useLogoSwap}; anything that doesn't gets the house cadence.
 */
export const LOGO_ROTATE_HOLD_BRISK = 3600
 ...
 * social-proof walls both call it), so they roll on one cadence unless a wall asks for another:
 * `hold` is the per-wall opt-out, and {@link LOGO_ROTATE_HOLD_BRISK} is the only other tempo in the
 * house. See memory `logo-swap-roll-motion`.
 */
export function useLogoSwap(hold: number = ROTATE_HOLD) {
...
/** The brand a slot shows on a given swap: the set walks forward in {@link LOGO_ROTATE_SLOTS} strides. */
function brandAt(index: number, step: number) {
  const n = PLACEHOLDER_BRANDS_VARIED.length
  return PLACEHOLDER_BRANDS_VARIED[(index + step * LOGO_ROTATE_SLOTS) % n]
}

/**
 * One rotating logo slot: rolls to a new brand as the shared `swap` counter ticks (drive it
 ...
 *
 * `cycle` walks the whole set instead of toggling one pair: see the note on the prop. It belongs to
 * walls on EQUAL GRID TRACKS only.
 */
export function RotatingPlaceholderLogo({
  ...
  mono?: boolean
  /**
   * Walk FORWARD through the set rather than toggling the same pair. Off, the slot alternates brand
   * `index` with `index + LOGO_ROTATE_SLOTS`, so the wall has exactly two states and its width is
   * fixed by that pair; on, each swap brings a brand nobody has seen yet (48 brands on a stride of 8
   * = six distinct sets before it comes round), which is what lets a brisk hold read as variety
   * instead of a blinking toggle.
   *
   * The catch is the width: the slot is sized to the two lockups currently in it, so a walking slot
   * re-measures on every swap. Inside EQUAL GRID TRACKS that is invisible (the track owns the width);
   * inside a `flex-wrap` row it would nudge every neighbour on each tick, so those rows stay on the
   * fixed pair. Opt in per wall, never by default.
   */
  cycle?: boolean
  className?: string
}) {
  // The two layers alternate by parity, and THAT alternation is what restarts the roll keyframes
  // (the class lands on the other element), so it is load-bearing: don't collapse it to a fixed
  // incoming/outgoing pair. Each layer therefore holds the set from the swap it was last shown on,
  // which keeps the pair on screen always {incoming, outgoing}.
  const firstVisible = swap % 2 === 0
  const step = cycle ? swap : swap % 2
  const first = brandAt(index, firstVisible ? step : step - 1)
  // At rest (swap 0) the hidden layer holds the NEXT set, the one the first tick rolls in.
  const second = brandAt(index, firstVisible ? Math.max(step - 1, 1) : step)
  return (
    <span
      className={cn(
        "inline-grid",
