...
export const pricingComparisonVariants = tv({
  slots: {
    // The shell: a single contour with no fill (fill is elevation, docs/FOUNDATIONS.md), so it
    // takes the ground it sits on. The edge is an `--edge` ring plus the xs lift, not a border
    // ("Shadows over borders"): a box-shadow sits outside the clip and takes no layout.
    // `overflow-hidden` clips the featured frame to the rounded corners; `overflow-x-auto` lets
    // the matrix scroll sideways on narrow screens.
    root: "w-full overflow-hidden rounded-2xl text-card-foreground shadow-xs ring-1 ring-edge",
    ...
    // Header. Sticky so the plans stay in view while you scan a long feature list; it paints the
    // ground the table sits on (`--surface`, else the page), opaque, so scrolled rows and the
    ...
    planPeriod: "text-xs font-medium tabular-nums text-muted-foreground",
...
// measured overlays (frame/tint); the header is opaque (sticky, painting the ground), so the tint overlay
