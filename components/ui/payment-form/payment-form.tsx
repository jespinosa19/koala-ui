 ...
 *
 * The card paints no fill (fill is elevation, docs/FOUNDATIONS.md), so nested controls take the
 * ground it sits on. Never render a real card number unmasked. This is presentation only.
 */
export const paymentFormVariants = tv({
  slots: {
    ...
    root: "flex w-full max-w-md flex-col rounded-2xl text-card-foreground shadow-xs ring-1 ring-edge",
    ...
    summary: "flex items-center justify-between rounded-xl bg-muted px-4 py-3 [--surface:var(--muted)]",
    ...
    summaryPanel: "flex flex-col gap-3 rounded-xl bg-muted p-4 [--surface:var(--muted)]",
...
export function PaymentForm({
  ...
  const slots = paymentFormVariants({ density: resolvedDensity })
  ...
  if (status === "success") {
    return (
      <div data-slot="payment-form" className={slots.root({ className })} {...props}>
