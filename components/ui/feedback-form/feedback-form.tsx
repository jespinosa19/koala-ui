 ...
 *
 * The card paints no fill (fill is elevation, docs/FOUNDATIONS.md), so the nested
 * ToggleGroup/Textarea take the same ground the card sits on.
 */
export const feedbackFormVariants = tv({
  slots: {
    ...
    root: "flex w-full max-w-sm flex-col rounded-2xl text-card-foreground shadow-xs ring-1 ring-edge",
...
export function FeedbackForm({
  ...
  const slots = feedbackFormVariants({ density: resolvedDensity })
  ...
  if (status === "success") {
    return (
      <div data-slot="feedback-form" className={slots.root({ className })} {...props}>
    ...
    <div data-slot="feedback-form" className={slots.root({ className })} {...props}>
      ...
      <Form density={resolvedDensity} onSubmit={handleSubmit}>
        ...
        <p
          ...
          aria-live="polite"
        >
          {/* The caption flips between the prompt and five rating words, all different widths,
              so it rolls rather than cutting (the same treatment PasswordStrength gives its
              Weak → Strong read-out). The colour already transitions; without this the word
              underneath it snapped while the tint eased. */}
          <AnimatedLabel swapKey={activeLabel ?? "none"}>
        ...
        <FormActions align="stack">
          <Button type="submit" loading={status === "loading"} disabled={!rating}>
            <ArrowUp weight="bold" />
            {action}
          </Button>
        </FormActions>
