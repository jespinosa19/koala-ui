 ...
 * falling back to `--background` on a plain page; the opt-in ring-edged `container` rides it too.
 */
export const tableVariants = tv({
  ...
  variants: {
    // The surface treatment. `minimal` is the default: no contour, radius, or fill, with the edge
    // cells pulled flush so the rows read as lined records and drop into whatever already frames
    // them. `container` is the opt-in: the ring-edged, rounded contour for when the table stands on
    // its own. Neither paints a fill of its own (fill is elevation, docs/FOUNDATIONS.md); both
    // rebase every opaque surface via `--table-surface` onto the ground they sit on, so
    // stripe/hover/sticky/pinned stay opaque in either.
    variant: {
      minimal: {
      ...
      // radius (docs/FOUNDATIONS.md, "Shadows over borders").
      container: {
        root: "rounded-xl text-card-foreground ring-1 ring-border [--table-surface:var(--surface,var(--background))]",
      },
    },
...
export interface TableProps
  ...
  /** Surface treatment: `minimal` (no contour/radius/fill, edge cells flush, lined records on the
   *  page; the default) or `container` (the ring-edged, rounded card). @default "minimal" */
