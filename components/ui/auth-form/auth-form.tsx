...
import { useDensity, ControlSizeProvider, type Density } from "@/lib/density"
...
export const authFormVariants = tv({
  slots: {
    ...
    socialLink:
      "flex items-center gap-1.5 rounded-sm font-medium outline-none transition-colors duration-fast ease-out hover:text-foreground focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface,var(--background))]",
    labelRow: "flex items-center justify-between",
    forgot:
      "rounded-sm text-sm font-medium text-brand outline-none transition-colors duration-fast ease-out hover:text-brand/80 focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface,var(--background))]",
    ...
    footerLink:
      "rounded-sm font-medium text-brand outline-none transition-colors duration-fast ease-out hover:text-brand/80 focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface,var(--background))]",
  },
  variants: {
    // `card` is the standalone ring-edged block (the default); `bare` strips the chrome (no
    // edge, shadow, max-width, or padding) to embed inside a page shell. Neither paints a fill
    // (fill is elevation, docs/FOUNDATIONS.md), so fields and focus-ring offsets take the ground
    // the form sits on, the same in both. The edge is an `--edge` ring plus the xs lift, not a
    // border (docs/FOUNDATIONS.md, "Shadows over borders").
    variant: {
      card: {
        root: "max-w-sm rounded-2xl text-card-foreground shadow-xs ring-1 ring-edge",
      },
      bare: {},
    },
...
function SocialRow({
  ...
  return (
    // The row sits OUTSIDE the `<Form>` below (it submits nothing), so it never inherited the
...
export interface LoginFormProps extends Omit<React.ComponentProps<"div">, "onSubmit" | "title"> {
  ...
  /** `card` (default) ships the standalone ring-edged block; `bare` drops the chrome to embed inside a page shell (e.g. a `SplitPane`). */
  variant?: "card" | "bare"
...
export function LoginForm({
  ...
  return (
    <div data-slot="login-form" className={slots.root({ className })} {...props}>
...
export function SignUpForm({
  ...
  return (
    <div data-slot="sign-up-form" className={slots.root({ className })} {...props}>
      {(title != null || description != null) && (
        <div className={slots.header()}>
          {title != null && <h3 className={slots.title()}>{title}</h3>}
          {description != null && <p className={slots.description()}>{description}</p>}
...
export function ProviderForm({
  ...
  return (
    <div data-slot="provider-form" className={slots.root({ className })} {...props}>
      {(title != null || description != null) && (
        <div className={slots.header()}>
          {title != null && <h3 className={slots.title()}>{title}</h3>}
          {description != null && <p className={slots.description()}>{description}</p>}
      ...
      {/* The full-width provider stack is the auth CTA, so it sits at the same `lg` tier as the
          form's fields; the band imposes it once instead of each button carrying a `size`. */}
      <ControlSizeProvider size="lg">
        <div className={slots.providerStack()} data-control-size="lg">
      ...
      {showEmail && (
        <>
          ...
          <Form density={resolvedDensity} onSubmit={handleEmail}>
            ...
            <FormActions align="stack">
              <Button type="submit" loading={loading} disabled={gated}>
                {emailAction}
              </Button>
            </FormActions>
          </Form>
