...
export const orderSummaryVariants = tv({
  slots: {
    ...
    // image-outline so a photo never floats edgeless. The outline rides on ::after, OVER the
    // photo: a ring on the tile itself is an inset box-shadow, which paints under the filling
    // <img> and never shows (memory `inset-ring-under-children`). Transparent fill so a glyph
    // blends into the surface; an <img> rounds to the tile itself (`rounded-[inherit]`) rather
    // than relying on `overflow-hidden`, which would clip the count pill parked outside the
    // corner. Holds an <img>, a Phosphor glyph, or nothing (a bare outlined tile).
    thumbnail:
      "relative grid shrink-0 place-items-center rounded-lg text-muted-foreground [&>img]:size-full [&>img]:rounded-[inherit] [&>img]:object-cover [&>svg]:size-5 after:pointer-events-none after:absolute after:inset-0 after:rounded-[inherit] after:ring-1 after:ring-inset after:ring-black/10 dark:after:ring-white/10",
    // The qty pill parked on the tile's corner. The neutral inverse pair (bg-foreground /
    // text-background) reads on any theme; a ring in the ground's color cuts it out of the photo.
    // `z-10` lifts it over the tile's ::after outline, which comes later in paint order.
    count:
      "absolute -right-2 -top-2 z-10 flex h-5 min-w-5 items-center justify-center rounded-full bg-foreground px-1 text-[11px] font-semibold tabular-nums text-background ring-2 ring-[var(--surface,var(--background))]",
    ...
    // Promo-code band: wraps a canonical InputGroup (field + Apply button) at the checkout
    // control tier (lg), so the field lines up with the "Checkout" CTA under the totals.
    promo: "",
  ...
  variants: {
    variant: {
      // A card wrapping the summary (a checkout sidebar). No internal dividers: bands
      // are set apart by whitespace, and the card insets them with padding (see compounds).
      card: {
        // A contour with no fill (fill is elevation, docs/FOUNDATIONS.md): the promo InputGroup,
        // its field and any Select take the same ground the summary sits on. The edge is an
        // `--edge` ring plus the xs lift, not a border ("Shadows over borders").
...
export interface OrderSummaryPromoProps extends React.ComponentProps<"div"> {
  /**
   * Control height imposed on the band's field + button. Checkout controls sit at the big tier,
   * the way a `Form` puts its fields there (see lib/density.tsx): a promo field is a real form
   * control, not a toolbar filter, and at `lg` it lines up with the "Checkout" CTA below. A
   * `size` on the control itself still wins, so this is a default, not an override. @default "lg"
   */
  size?: ControlSize
}

/** Promo band: a slot for a canonical InputGroup (field + Apply button), sized to the CTA. */
export function OrderSummaryPromo({
  ...
  return (
    <div
      data-slot="order-summary-promo"
      data-control-size={size}
      className={slots.promo({ className })}
      {...props}
    >
...
export interface OrderSummaryFooterProps extends React.ComponentProps<"div"> {
  /**
   * Control height imposed on the band's CTA. Same tier as {@link OrderSummaryPromo}: the two
   * bands bracket the totals, so a checkout button that fell back to the ambient density would
   * read a step smaller than the promo field right above it. @default "lg"
   */
  size?: ControlSize
}

/** CTA band: the "Checkout" Button plus a trust note. A slot the consumer fills, sized to match. */
export function OrderSummaryFooter({
  ...
  return (
    <div
      data-slot="order-summary-footer"
      data-control-size={size}
