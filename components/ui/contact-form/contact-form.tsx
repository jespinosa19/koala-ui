 ...
 * blend with the panel (the --surface contract). The `bare` variant drops the card chrome (edge,
...
export const contactFormVariants = tv({
  ...
  variants: {
    // `card` is the standalone ring-edged block (the default); `bare` strips the chrome (no edge,
    // shadow, max-width, or padding) to embed inside a page shell that owns its surface. Neither
    // paints a fill: fill is elevation (docs/FOUNDATIONS.md), so the fields take the ground the
    // form sits on. The edge is an `--edge` ring plus the xs lift, not a border ("Shadows over
    // borders"). With `slots`, each variant value maps slot→classes (an object), not a string.
    variant: {
      card: {
        root: "max-w-lg rounded-2xl text-card-foreground shadow-xs ring-1 ring-edge",
      },
      bare: {},
    },
...
interface ContactFormBaseProps extends Omit<React.ComponentProps<"div">, "onSubmit" | "title"> {
  ...
  /** `card` (default) ships the standalone ring-edged block; `bare` drops the chrome to embed inside a page shell. */
  variant?: "card" | "bare"
...
export function ContactForm({
  ...
  const controlSize = useControlSize(undefined, resolvedDensity)
  ...
  if (status === "success") {
    return <SuccessPanel data-slot="contact-form" slots={slots} name={data.name} className={className} {...props} />
  }

  return (
    <div data-slot="contact-form" className={slots.root({ className })} {...props}>
      <FormHeader slots={slots} title={title} description={description} />

      {/* A bare Form imposes `lg` (40px), which overpowers a 16px-padded card. The block passes
          the density's default control size instead (compact → sm 32px, comfortable → md 36px),
          so the inputs, selects, phone, textarea, and submit button share one height that scales
          with the card around them. */}
      <Form density={resolvedDensity} size={controlSize} onSubmit={handleSubmit}>
        <FieldRow>
          <Field>
...
export function LeadForm({
  ...
  if (status === "success") {
    return <SuccessPanel data-slot="lead-form" slots={slots} name={data.firstName} className={className} {...props} />
  }

  return (
    <div data-slot="lead-form" className={slots.root({ className })} {...props}>
      <FormHeader slots={slots} title={title} description={description} />
...
export function SupportForm({
  ...
  if (status === "success") {
    return (
      <SuccessPanel data-slot="support-form"
        slots={slots}
        name={data.name}
        className={className}
        text={
  ...
  return (
    <div data-slot="support-form" className={slots.root({ className })} {...props}>
      <FormHeader slots={slots} title={title} description={description} />
