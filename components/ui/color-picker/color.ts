...
// ─── HSV ⇄ HSL ──────────────────────────────────────────────────────────────────
//
// HSL is a display format only: the picker still *thinks* in HSV (the square maps to it
// directly), but designers read and paste HSL, so the fields row offers it. The two share a hue,
// which is why the conversion never has to round-trip through RGB.
...
export function hsvaToHsla({ h, s, v, a }: Hsva): Hsla {
  ...
  const vv = v / 100
  const l = vv * (1 - sv / 2)
  // At pure black or pure white the saturation is undefined (the divisor collapses); report 0
  // rather than dividing by zero, and let `reconcileHsva` preserve the working hue on the way back.
...
export function hslaToHsva({ h, s, l, a }: Hsla): Hsva {
  ...
  return { h: round(h), s: round(sv * 100), v: round(v * 100), a }
}
