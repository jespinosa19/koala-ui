    ...
    <div data-slot="color-picker-image" className={cn("flex flex-col gap-3", className)}>
      ...
      {image.src && (
        <ToggleGroup
          ...
          className={slots.segmentedRow()}
          // Radix's root is `role="group"`, but its `type="single"` items are real `role="radio"`.
          role="radiogroup"
          data-slot="color-picker-image-fit"
          aria-label="Image fit"
        >
          <ToggleGroupItem value="cover" className={slots.segmentedItem()}>
            Cover
          </ToggleGroupItem>
          <ToggleGroupItem value="contain" className={slots.segmentedItem()}>
            Contain
          </ToggleGroupItem>
        </ToggleGroup>
      )}
